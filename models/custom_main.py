...
from .custom_architecture import Custom_Arch_Model
import random
import math
...
class Custom_CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        self.emb_total_dim = self.emb_time_dim + self.emb_feature_dim + len(self.cycles_alphas) * 2 #+ self.emb_feature_dim
        ...
        self.diffmodel = Custom_Arch_Model(config_diff, input_dim)
    ...
    def calc_loss(
        ...
        # First Call
        total_initial_input = self.set_input_to_diffmodel(noisy_data, observed_data, cond_mask)
        initial_predicted = self.diffmodel(total_initial_input, side_info, t)  # (B,K,L)
    ...
    def __impute(self, observed_data, cond_mask, side_info, n_samples):
        B, K, L = observed_data.shape
    ...
    def forward(self, batch, is_train=1):
        ...
        if is_train == 0:
            ...
            coin = random.uniform(0, 1)
            if coin < 0.5:
                #print(observed_mask.shape, gt_mask.shape)
...
class Custom_CSDI_WaterQual(Custom_CSDI_base):
    def __init__(self, config, device, target_dim=12):
        super(Custom_CSDI_WaterQual, self).__init__(target_dim, config, device)
