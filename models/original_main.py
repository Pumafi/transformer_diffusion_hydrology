...
from .original_architecture import diff_CSDI
import random
...
class CSDI_Forecasting(CSDI_base):
    ...
    def generate(self, observed_data, observed_mask, observed_tp, n_samples):
        ...
        #feature_id = feature_id.to(self.device).float()
        ...
        side_info = self.get_side_info(observed_tp, observed_mask, feature_id)

        return self.impute(observed_data, observed_mask, side_info)
    ...
    def evaluate(self, batch, n_samples, forecasting_mode=True, ratio_to_mask=0.5):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        if not forecasting_mode:
            ratio_to_keep = 1. - ratio_to_mask
            timesteps_to_keep = int(ratio_to_keep * observed_mask.shape[1])
            gt_mask = torch.zeros_like(observed_mask)
            for i in range(observed_mask.shape[-1]):
                ...
                gt_mask[:, observation_indices, i] = observed_mask[:, observation_indices, i]
